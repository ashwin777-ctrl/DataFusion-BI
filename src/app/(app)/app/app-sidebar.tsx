...
export function AppSidebar() {
  ...
  return (
    <aside
      ...
      className={`${
        collapsed ? "w-20" : "w-64"
      } transition-all duration-300 ease-in-out border-r border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/60 backdrop-blur-xl flex flex-col shrink-0 sticky top-0 h-screen z-30`}
    >
      {/* Brand Logo */}
          ...
          {!collapsed && (
            <div className="flex flex-col overflow-hidden">
              <span className="text-base font-bold tracking-tight text-slate-900 dark:text-white whitespace-nowrap">
                DataFusion <span className="text-indigo-600 dark:text-indigo-400 font-semibold text-xs">BI</span>
              </span>
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors"
        >
          {collapsed ? (
