...
import { listPostgresTables, parsePgConnectionString } from "@/lib/engine/ingest-postgres";
  ...
  try {
    ...
    if (!host || typeof host !== "string" || !host.trim()) {
      return NextResponse.json(
        { error: "Database host is required (e.g. 127.0.0.1 or cloud host)" },
        { status: 400 },
      );
    }
    if (!database || typeof database !== "string" || !database.trim()) {
      return NextResponse.json(
        { error: "Database name is required (e.g. postgres)" },
        { status: 400 },
      );
    }
    if (!user || typeof user !== "string" || !user.trim()) {
      return NextResponse.json(
        { error: "Database username is required (e.g. postgres)" },
        { status: 400 },
      );
    }
